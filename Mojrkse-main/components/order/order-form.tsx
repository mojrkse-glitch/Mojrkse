...
const walletMethodId = "wallet-balance";
...
function getPaymentGuide(method?: PaymentMethod | null): PaymentGuide | null {
  ...
  switch (method.slug) {
    case "syriatel-cash":
    case "mtn-cash":
      return { unitLabel: "ل.س", amountLabel: "المبلغ المحول بالليرة السورية", hint: "اكتب القيمة المحولة كما ظهرت في العملية." };
    case "shamcash":
      return { unitLabel: "حسب العملة", amountLabel: "مبلغ التحويل عبر شام كاش", hint: "اختر العملة ثم أدخل المبلغ بنفس العملة.", requiresCurrencyChoice: true };
    case "usdt":
    case "ltc":
      return { unitLabel: "USD", amountLabel: "المبلغ المطلوب تحويله بالدولار", hint: "أدخل قيمة التحويل بالدولار قبل تنفيذ الدفع." };
    default:
      return { unitLabel: "USD", amountLabel: "مبلغ التحويل", hint: "أدخل المبلغ الذي ستقوم بتحويله بهذه الوسيلة." };
  }
}

export function OrderForm({ service, paymentMethods, settings, walletBalance = 0 }: Props) {
  const router = useRouter();
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<string>(walletBalance > 0 ? walletMethodId : (paymentMethods[0]?.id || ""));
  ...
  const isWalletPayment = selectedPaymentMethod === walletMethodId;
  const isHandDelivery = !isWalletPayment && Boolean(selectedMethod?.is_hand_delivery);
  const paymentGuide = isWalletPayment ? null : getPaymentGuide(selectedMethod);
  ...
  useEffect(() => {
    if (!selectedMethod || selectedMethod.is_hand_delivery || isWalletPayment) return;
    const nextCurrency = selectedMethod.slug === "shamcash" ? (dynamicValues.payment_transfer_currency || "syp") : selectedMethod.slug === "usdt" || selectedMethod.slug === "ltc" ? "usd" : selectedMethod.slug === "syriatel-cash" || selectedMethod.slug === "mtn-cash" ? "syp" : (dynamicValues.payment_transfer_currency || "usd");
    setDynamicValues((current) => ({ ...current, payment_transfer_currency: nextCurrency }));
  }, [selectedMethod?.id, isWalletPayment]);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setMessage(null);
    try {
      if (!isWalletPayment && paymentGuide && !isHandDelivery && !dynamicValues.payment_transfer_amount?.trim()) {
        throw new Error("يرجى إدخال مبلغ التحويل حسب وسيلة الدفع المختارة.");
      }
      if (!isWalletPayment && selectedMethod?.slug === "shamcash" && !dynamicValues.payment_transfer_currency) {
      ...
      <div className="space-y-5 rounded-2xl border border-border bg-white/[0.02] p-4 md:p-6">
        ...
        <div>
          ...
          <select id="payment-method" className="flex h-11 w-full rounded-2xl border border-border bg-white/[0.03] px-4 text-sm text-foreground outline-none focus:border-primary/60" value={selectedPaymentMethod} onChange={(e) => setSelectedPaymentMethod(e.target.value)}>
            {walletBalance > 0 ? <option value={walletMethodId}>الدفع من رصيد المحفظة ({formatCurrency(walletBalance)})</option> : null}
        ...
        {isWalletPayment ? (
          <div className="rounded-2xl border border-primary/20 bg-primary/10 p-4 text-sm">
            <p className="font-semibold text-foreground">الدفع من المحفظة</p>
            <p className="mt-2 leading-7 text-muted-foreground">سيتم إنشاء الطلب وخصم الرصيد تلقائيًا مباشرة من المحفظة دون الحاجة إلى إثبات دفع.</p>
            <p className="mt-2 font-semibold text-primary">رصيدك الحالي: {formatCurrency(walletBalance)}</p>
          </div>
        ) : selectedMethod ? (
          <div className="rounded-2xl border border-border bg-background/70 p-4 text-sm">
            ...
            {selectedMethod.wallet_address ? <div className="mt-3 rounded-xl border border-primary/20 bg-primary/10 p-3 font-mono text-xs text-primary">{selectedMethod.wallet_address}</div> : null}
          </div>
        ) : null}

        {!isWalletPayment && paymentGuide ? (
          <div className="rounded-2xl border border-border bg-background/70 p-4">
            ...
            <div>
              ...
              {selectedMethod?.slug === "syriatel-cash" || selectedMethod?.slug === "mtn-cash" ? <p className="mt-2 text-xs text-muted-foreground">القيمة التقريبية حسب سعر الصرف الحالي: {(pricing.finalUsdPrice * settings.exchangeRate).toLocaleString("ar-SY")} ل.س</p> : null}
            </div>
          </div>
        ) : null}

        {!isWalletPayment && isHandDelivery ? <a href={generateWhatsAppLink(siteConfig.whatsappMessage, siteConfig.whatsappPhone)} target="_blank" rel="noreferrer" className="inline-flex h-11 w-full items-center justify-center rounded-2xl bg-secondary px-5 text-sm font-semibold text-secondary-foreground">فتح واتساب للتنسيق</a> : null}
        ...
        <Button type="submit" className="w-full" disabled={loading}>{loading ? "جارٍ إرسال الطلب..." : isWalletPayment ? "إنشاء الطلب وخصم الرصيد" : "إرسال الطلب الآن"}</Button>
      </div>
    </form>
  );
}

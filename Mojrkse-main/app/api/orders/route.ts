...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!serviceId || !paymentMethodId) return NextResponse.json({ message: "بيانات الطلب غير مكتملة." }, { status: 400 });

    const [serviceRes, fieldsRes, paymentRes, settingsRes] = await Promise.all([
      ...
      supabase.from("payment_methods").select("*").eq("id", paymentMethodId).eq("is_enabled", true).maybeSingle(),
      supabase.from("settings").select("key, value")
    ]);
    if (serviceRes.error || !serviceRes.data) return NextResponse.json({ message: "الخدمة غير متاحة حاليًا." }, { status: 404 });
    if (paymentRes.error || !paymentRes.data) return NextResponse.json({ message: "وسيلة الدفع غير متاحة." }, { status: 404 });
    ...
    const paymentMethod = paymentRes.data;
    ...
    if (paymentProof) {
      ...
      paymentProofPath = uploaded.path; paymentProofMeta = { fileName: uploaded.fileName, mimeType: uploaded.mimeType, fileSize: uploaded.fileSize }; uploadedFiles.push({ bucket: "payment-proofs", path: uploaded.path });
    }
    if (reference) {
      ...
      referenceFilePath = uploaded.path; uploadedFiles.push({ bucket: "order-reference-files", path: uploaded.path });
    }
